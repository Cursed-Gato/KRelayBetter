...
namespace MapCacher
{
    internal static class Extensions
    {
        ...
        public static List<T> Shift<T>(this List<T> list, int count)
        {
            ...
            var output = list.GetRange(0, count);
    ...
    public class MapCacher : IPlugin
    {
        ...
        public void Initialize(Proxy proxy)
        {
            ...
            proxy.HookCommand("mapcacher", (client, cmd, args) =>
            {
                Config.Default.enabled = !Config.Default.enabled;
                Config.Default.Save();
                client.SendToClient(PluginUtils.CreateOryxNotification("MapCacher",
                    "Map resending is now " + (Config.Default.enabled ? "enabled" : "disabled")));
            });
...
#if DEBUG
                PluginUtils.Log("Map Cacher", "Loaded cached map {0}, {1} cached tiles queued", mapInfo.Name,
                    SendQueues[client].Count);
#endif
            }
            else
            {
        ...
        private void OnUpdate(Client client, Packet p)
        {
            ...
            {
                var newTiles = new List<Tile>(update.Tiles);
                SendQueues[client].Shift(Math.Min(MaxTilesPerPacket, Math.Max(0, MaxTilesPerPacket - newTiles.Count)))
                    .ForEach(tile => newTiles.Add(tile));
